...
export const MainLobbyView: React.FC<MainLobbyViewProps> = ({
  ...
  const [activeTab, setActiveTab] = useState<'quick_play' | 'online' | 'pass_play' | 'vs_bots' | 'stakes'>(
    'quick_play'
  );
  ...
  const [publicRooms, setPublicRooms] = useState<
    { roomId: string; playerCount: number; hostName: string; isCompetitive?: boolean }[]
  >([]);

  useEffect(() => {
    // Fetch live available rooms
  ...
  const handleStartPassAndPlay = (count: number) => {
    const chosenColors = ALL_COLORS.slice(0, count);
  ...
  return (
    <div className="w-full max-w-5xl mx-auto space-y-5 animate-fadeIn">
        ...
        <div className="relative z-10 flex flex-col md:flex-row items-start md:items-center justify-between gap-5">
          <div className="space-y-2 max-w-xl">
            ...
            <p className="text-xs sm:text-sm text-slate-400 leading-relaxed">
              Play classic Ludo with friends, challenge players worldwide, or enter real-stakes UGX cash battles powered by Pesapal.
            </p>
          </div>
      ...
      {/* Mode Selector Tabs */}
      <div className="grid grid-cols-2 sm:grid-cols-5 gap-2 font-bold text-xs">
        {[
          { id: 'quick_play', label: 'Quick 1v1 / 4P', icon: Play, color: 'text-emerald-400' },
          { id: 'stakes', label: 'Cash Stakes (UGX)', icon: Flame, color: 'text-amber-400' },
          { id: 'online', label: 'Online Rooms', icon: Globe, color: 'text-sky-400' },
          { id: 'pass_play', label: 'Pass & Play', icon: Users, color: 'text-purple-400' },
          { id: 'vs_bots', label: 'VS AI Bots', icon: Bot, color: 'text-rose-400' },
        ].map((tab) => {
          const Icon = tab.icon;
          ...
          <div className="space-y-6">
            <div className="text-center max-w-md mx-auto space-y-1">
              ...
              <p className="text-xs text-slate-400">
                Jump right into an instant match with standard official Ludo rules.
              </p>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 max-w-2xl mx-auto">
              ...
              <div className="p-5 rounded-2xl bg-slate-950/80 border border-slate-800 hover:border-emerald-500/60 transition flex flex-col justify-between space-y-4">
                <div className="space-y-2">
                  ...
                  <p className="text-xs text-slate-400">
                    Head-to-head 1v1 battle (Red vs Yellow). High speed, high tactical capture opportunities.
                  </p>
                </div>
              ...
              <div className="p-5 rounded-2xl bg-slate-950/80 border border-slate-800 hover:border-sky-500/60 transition flex flex-col justify-between space-y-4">
                <div className="space-y-2">
                  ...
                  <p className="text-xs text-slate-400">
                    Full 4-corner Ludo mayhem (Red, Green, Yellow, Blue). First player to home all 4 pawns wins!
                  </p>
                </div>
          ...
          <div className="space-y-6 max-w-2xl mx-auto">
            ...
            <div className="p-4 bg-slate-950 border border-slate-800 rounded-2xl space-y-3">
                ...
                <div className="flex items-center gap-2">
                  <button
                    ...
                    className={`px-3 py-1 rounded-lg text-xs font-bold transition ${
                      stakePlayerCount === 2 ? 'bg-amber-500 text-slate-950 font-black' : 'bg-slate-800 text-slate-400'
                    }`}
                  >
                    1v1 (2 Players)
                  </button>
                  <button
                    ...
                    className={`px-3 py-1 rounded-lg text-xs font-bold transition ${
                      stakePlayerCount === 4 ? 'bg-amber-500 text-slate-950 font-black' : 'bg-slate-800 text-slate-400'
                    }`}
                  >
                    4 Players
                  </button>
              ...
              <div className="pt-3 border-t border-slate-800/80 flex items-center justify-between text-xs">
                <div>
                  <span className="text-slate-500 block">Total Pool: UGX {(selectedStake * stakePlayerCount).toLocaleString()}</span>
                  <span className="text-[10px] text-amber-400">Platform Rake (10%): UGX {stakeQuote.platformFee.toLocaleString()}</span>
                </div>
              ...
              <button
                onClick={() => {
                  if (joinCode.trim().length >= 4) {
                    onJoinOnlineRoom(joinCode.trim(), profile.name, profile.avatar, profile.preferredColor);
                  }
                }}
        ...
        {activeTab === 'pass_play' && (
          <div className="space-y-5 max-w-xl mx-auto">
            <div className="text-center space-y-1">
              ...
              <p className="text-xs text-slate-400">
                Play on a single device with friends and family. Hand over the phone on each turn!
              </p>
            </div>
        ...
        {activeTab === 'vs_bots' && (
          <div className="space-y-5 max-w-xl mx-auto">
            <div className="text-center space-y-1">
              ...
              <p className="text-xs text-slate-400">
                Sharpen your tactical pawn routing against smart artificial intelligence.
              </p>
            </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <button
                ...
                className="py-3 rounded-xl bg-rose-600 hover:bg-rose-500 text-white font-black text-xs shadow-lg transition"
              >
                1 Human + 3 AI Bots
              </button>
            </div>
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 pt-2 text-xs text-slate-400">
        ...
        <button
          onClick={onOpenAdmin}
          className="px-3 py-1.5 rounded-xl bg-slate-900 hover:bg-slate-800 border border-slate-700 text-amber-400 font-bold text-xs flex items-center gap-1.5 transition"
        >
